...
window.cargarIndicadores = async function () {
    try {
        ...
        const ufText = '$' + Number(data.uf).toLocaleString('es-CL');
        const dolarText = '$' + Number(data.dolar).toLocaleString('es-CL');
        const euroText = '$' + Number(data.euro).toLocaleString('es-CL');
        const utmText = '$' + Number(data.utm).toLocaleString('es-CL');
        const ipcText = data.ipc + '%';
        ...
        updateElements(['#banner-uf', '#dash-uf'], ufText);
        updateElements(['#banner-dolar', '#dash-dolar', '#nav-dolar'], dolarText);
        updateElements(['#dash-euro', '#nav-euro'], euroText);
        updateElements(['#banner-utm', '#dash-utm', '#nav-utm'], utmText);
        updateElements(['#nav-ipc'], ipcText);
        updateElements(['#banner-fecha', '#dash-fecha', '#nav-fecha'], fechaText);

    } catch (error) {
        console.error('Error al cargar indicadores:', error);
        const fallbackText = "No disp.";
        const selectors = [
            '#banner-uf', '#dash-uf',
            '#banner-dolar', '#dash-dolar', '#nav-dolar',
            '#dash-euro', '#nav-euro',
            '#banner-utm', '#dash-utm', '#nav-utm',
            '#nav-ipc',
...
document.addEventListener('DOMContentLoaded', window.cargarIndicadores);
